...
router.get('/', async (req, res, next) => {
  try {
    ...
    const result = await passService.listPasses(filters);
...
router.post('/', async (req, res, next) => {
  try {
    const result = await passService.createPass(req.body, req.session.userId);
...
router.get('/buscar-visitantes', async (req, res, next) => {
  try {
    const q = (req.query.q || '').trim();
    if (q.length < 2) return res.json([]);
    const results = await passService.searchVisitors(q);
    res.json(results);
  } catch (err) { next(err); }
});
...
router.post('/:id', async (req, res, next) => {
  try {
    ...
    const result = await passService.updatePass(id, req.body);

    if (!result.ok) {
      const pass = { ...(await passService.getPassById(id)), ...req.body, id };
      return res.render('passes/form', {
        title: 'Editar pase',
        active: 'passes',
        pass,
    ...
    realtime.notifyPassChange('update', await passService.getPassById(id));
    res.redirect('/pases');
  } catch (err) { next(err); }
});
...
router.post('/:id/eliminar', async (req, res, next) => {
  try {
    const id = req.params.id;
    const ok = await passService.deletePass(id);
    if (ok) {
...
router.post('/:id/reenviar-correo', async (req, res, next) => {
  try {
    const id = req.params.id;
